...
def research_agent(category: dict) -> list[dict] | None:
    ...
    # Deduplicate by first 40 chars of headline and skip already processed ones
    ...
    for s in stories:
        ...
        if key not in seen:
            # Check if this precise URL was already processed by a previous bot run
            cur = db.execute("SELECT processed FROM raw_stories WHERE id = ?", (s["url"],))
            row = cur.fetchone()
        ...
        try:
            ...
            for entry in feed.entries[:8]:
                ...
                if not any(kw in text for kw in keywords):
                    continue
                from .db import store_raw_story
