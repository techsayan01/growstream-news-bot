...
def _check_wordpress() -> CheckResult:
    ...
    try:
        ...
        # context=edit is required for WP to return roles + capabilities
        me = requests.get(
        ...
        # `roles` may be empty depending on JWT plugin version — informational only
        ...
        # --- Definitive publish-rights test: create a private draft, then delete it ---
        # This is the only reliable method when JWT plugin doesn't populate capabilities.
        ...
        if test_post.status_code == 201:
            # Immediately delete the test post
            ...
            # Unexpected status — treat as a warning but don't block (could be a theme/plugin conflict)
            return CheckResult(
                name="WordPress API",
                ok=True,
                message=(
